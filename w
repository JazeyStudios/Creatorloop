Absolutely! Adding more **detailed features** like banning/unbanning creators/hosts, viewing violation history, and quick links like "View Live Now" profiles for creators is definitely doable.

---

### Here’s the plan for these extra features:

**1. Ban / Unban functionality**

* Creators and Hosts can be banned/unbanned.
* Banned users show a red badge or strikethrough.
* Banned users cannot be assigned to new relationships (optional).

**2. Violation logs per creator/host**

* You can add/view violations (like rule breaches) per creator/host.
* Violation history stored as a list of text + timestamp.

**3. "View Live Now"**

* Button/link in creator row to open their TikTok profile or a placeholder URL.
* Opens in a new tab.

**4. Show banned status visually**

* Badges or icons next to names.

---

### I’ll expand the code with these features:

* Add `banned` boolean to creators and hosts
* Add `violations` array per creator/host, with date + reason
* Add buttons for Ban/Unban, View Violations, View Live Now
* Modal or expandable section to see violations
* Disable relationships with banned users (optional, can allow still)

---

### Here’s the **updated full HTML + JS** code with these features added:
...
aside class="sidebar"> <div class="logo">StreamCore</div> <nav> <button class="active" data-tab="dashboard">Dashboard</button> <button data-tab="creators">Creators</button> <button data-tab="hosts">Hosts</button> <button data-tab="relationships">Relationships</button> </nav>
...
const tr = document.createElement('tr');
tr.innerHTML = `        <td class="${bannedClass}">${h.name} ${bannedBadge}</td>         <td>${h.experience} years</td>         <td>           <button class="edit-btn" data-id="${h.id}">Edit</button>
          ${h.banned ?`<button class="unban-btn" data-id="${h.id}">Unban</button>`:`<button class="ban-btn" data-id="${h.id}">Ban</button>`}           <button class="violation-btn" data-id="${h.id}" data-type="host">Violations</button>           <button class="view-live-btn" data-id="${h.id}" data-name="${h.name}">View Live</button>           <button class="delete-btn" data-id="${h.id}">Delete</button>         </td>
      `;
...
div.innerHTML = `         <span class="${bannedClass}">${creator.name} &mdash; ${host.name}</span>         <button data-id="${r.id}">Remove</button>
      `;
